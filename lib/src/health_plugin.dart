...
class Health {
  static const MethodChannel _channel = MethodChannel('flutter_health_hs');
  ...
  Future<void> configure() async {
    _deviceId = Platform.isAndroid
        ...
        : (await _deviceInfo.iosInfo).identifierForVendor ?? 'unknown';
  }
  ...
  Future<void> revokePermissions() async {
    ...
    try {
      ...
      if (kDebugMode) {
        print('$runtimeType - Exception in revokePermissions(): $e');
      }
    }
  }
  ...
  Future<HealthConnectSdkStatus?> getHealthConnectSdkStatus() async {
    ...
    try {
      ...
      if (kDebugMode) {
        print('$runtimeType - Exception in getHealthConnectSdkStatus(): $e');
      }
      return null;
    }
  }
  ...
  Future<void> installHealthConnect() async {
    ...
    try {
      ...
      if (kDebugMode) {
        print('$runtimeType - Exception in installHealthConnect(): $e');
      }
    }
  }
  ...
  Future<List<HealthDataPoint>> getHealthDataFromTypes({
    ...
    if (dataPoints.length > threshold) {
      return await Isolate.run(() => removeDuplicates(dataPoints));
    }
  ...
  Future<List<HealthDataPoint>> _dataQuery(
    ...
    if (fetchedDataPoints != null && fetchedDataPoints is List) {
      ...
      // call the Isolate.run method to spawn an Isolate to do the parsing in a separate thread.
      if (fetchedDataPoints.length > thresHold) {
        return await Isolate.run(() => _parse(msg));
      }
      return _parse(msg);
    } else {
